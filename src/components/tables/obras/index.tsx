...
export function TabelaObras({ refreshTrigger }: TabelaObrasProps) {
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}api/obras/${selectedItem.id}/`, {
                method: 'DELETE',
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            });
    ...
    const filtered = data.filter((item: any) => {
        ...
        return (
            ...
            (item.endereco || "").toLowerCase().includes(search) ||
            (item.cidade || "").toLowerCase().includes(search) ||
            (item.qntdFuncionarios !== undefined && item.qntdFuncionarios !== null ? item.qntdFuncionarios.toString().toLowerCase() : "").includes(search)
        );
    });
    ...
    return (
        <div className="rounded-xl overflow-hidden border-[2px] border-[#E4E6F5]">
            ...
            <ModalConfirmacaoExclusao
                ...
                titulo="Excluir Obra"
                mensagem="Tem certeza que deseja excluir a obra"
